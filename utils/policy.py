...
moves =  ["a1b1",  "a1c1",  "a1d1",  "a1e1",  "a1f1",  "a1g1",  "a1h1",  "a1a2",
    ...
    "h2g1b", "h2h1q", "h2h1r", "h2h1b", "a7a8n", "a7b8n", "b7a8n", "b7b8n",
    "b7c8n", "c7b8n", "c7c8n", "c7d8n", "d7c8n", "d7d8n", "d7e8n", "e7d8n",
    "e7e8n", "e7f8n", "f7e8n", "f7f8n", "f7g8n", "g7f8n", "g7g8n", "g7h8n",
    "h7g8n", "h7h8n", "a2a1n", "a2b1n", "b2a1n", "b2b1n", "b2c1n", "c2b1n",
    "c2c1n", "c2d1n", "d2c1n", "d2d1n", "d2e1n", "e2d1n", "e2e1n", "e2f1n",
    "f2e1n", "f2f1n", "f2g1n", "g2f1n", "g2g1n", "g2h1n", "h2g1n", "h2h1n"]

moveIdx = dict(zip(moves, range(len(moves))))

def move2Vec(moveID):
    outIdx = moveIdx[moveID]
    moveVec = np.zeros(len(moves))
    moveVec[outIdx] = 1
    return(moveVec.astype('bool'))

def vec2Move(moveVec):
    outIdx = np.where(moveVec == 1)[0][0]
    return(moves[outIdx])

def testMove():
    mymove = 'e2e4'
    moveVec = move2Vec(mymove)
    outMove = vec2Move(moveVec)

    print(mymove == outMove)
